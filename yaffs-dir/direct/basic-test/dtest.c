...
void copy_in_a_file(const char *yaffsName,const char *inName)
{
	...
	{
		no = yaffs_write(outh,buffer,ni, 1);
		if(ni != no)
		{
...
void make_pattern_file(char *fn,int size)
{
	...
	yaffs_write(outh,"A",1, 1);

	for(i = 0; i < size; i+=256)
	{
		marker = ~i;
		yaffs_lseek(outh,i,SEEK_SET);
		yaffs_write(outh,&marker,sizeof(marker), 1);
	}
	yaffs_close(outh);
...
void fill_disk(const char *path,int nfiles)
{
	...
	for(n = 0; n < nfiles; n++)
	{
		...
		while ((result = yaffs_write(h,xx,600, 1)) == 600)
		{
		...
		for(i = 0; i <20000; i++)
		{
		   //r =        yaffs_lseek(b,i,SEEK_SET);
			...
			else
			{
				...
				r = yaffs_write(a,abuffer,1000, 1);
			}
			x++;
...
int overwrite_test(const char *path)
{
   ...
   b = yaffs_open(bname, O_CREAT | O_TRUNC | O_RDWR, S_IREAD | S_IWRITE);
   for(j= 0; j < 500; j++){
   	yaffs_write(b,bname,100, 1);
	a = yaffs_open(aname, O_CREAT | O_TRUNC | O_RDWR, S_IREAD | S_IWRITE);
   	for(i = 0; i < rand() % 20000; i++)
   		yaffs_write(a,&a,sizeof(a), 1);
	yaffs_close(a);
   }
...
int cache_bypass_bug_test(void)
{
	...
	yaffs_write(a,"abcdefghijklmnopqrstuvwxyz",20, 1);
...
int truncate_test(void)
{
	...
	yaffs_write(a,"abcdefghijklmnopqrstuvwzyz",26, 1);
...
void simple_rw_test(const char *mountpt)
{
	...
	for(i = 100000;i < 200000; i++){
		result = yaffs_write(h,&i,sizeof(i), 1);
...
#else
		while(yaffs_write(h0,xx,1000, 1) > 0){

		   yaffs_write(h1,xx,1000, 1);
		}
#endif
...
void small_overwrite_test(const char *mountpt,int nmounts)
{
	...
	for(i = 0; i < nmounts; i++){
		...
		for(j = 0; j < 1000000; j+=1000){
			...
			yaffs_write(h0,xx,7000, 1);
			yaffs_write(h1,xx,7000, 1);
...
void checkpoint_fill_test(const char *mountpt,int nmounts)
{
	...
	for(i = 0; i < nmounts; i++){
		...
		while(yaffs_write(h,c,50, 1) == 50, 1){}
...
int make_file2(const char *name1, const char *name2,int syz)
{
	...
	while(syz > 0 && n > 0){
		i = (syz > 2500) ? 2500 : syz;
		n = yaffs_write(h1,xx,i, 1);
		n = yaffs_write(h2,xx,i, 1);
		syz -= 500;
	}
	yaffs_close(h1);
	yaffs_close(h2);
...
{
	printf("-------------------In Basic UTime Test------------------\n");
	char name[100], name_2[100];
	int h;
	int result;
	int val1;
...
// Could Not Locate The Function 
	...
	printf ("File: Dtest.c => Func: Basic-UTime-Test => Trying to mount the file-system !!\n");
	yaffs_mount(mountpt);
	printf ("File: Dtest.c => Func: Basic-UTime-Test => Mount of the file-system done !!\n");
...
// File System Call  
	...
	printf ("Handle for the opened file = %d\n", h);

	strcpy(name_2,mountpt);
	strcat(name_2,"/");
	strcat(name_2,"ravi_file");
	yaffs_unlink(name_2);
	int handle_2 = yaffs_open(name_2,O_CREAT | O_TRUNC | O_RDWR, S_IREAD);
	printf ("Handle for the opened file = %d\n", handle_2);
...
int main_dtest(int argc, char *argv[])
{
