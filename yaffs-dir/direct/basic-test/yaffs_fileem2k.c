...
static int  CheckInit(void)
{
	...
	printf("SIZE_IN_MB=%d\nBLOCKS_PER_MB=%d\n",SIZE_IN_MB, BLOCKS_PER_MB);
	// NBlocks comes out to be 2 ^ 28 = 256 Mega Blocks
...
int yflash2_WriteChunkWithTagsToNAND(struct yaffs_dev *dev,int nand_chunk,const u8 *data, const struct yaffs_ext_tags *tags)
{
     ...
     //  printf ("Total number of blocks %d\n",filedisk.nBlocks );
      ...
      //  printf ("In yflash2_WriteChunkWithTagsToNAND\n"); 
	int written;
	int pos;
	int h;
	int i;
...
// Currently "dev->param.inband_tags" has been set to 0	
	if(dev->param.inband_tags)
		...
		if(data)
		{
			...
			lseek(h,pos,SEEK_SET);
			written = write(h,localBuffer,dev->data_bytes_per_chunk);
			memset(result_str, 0, 200);
                        sprintf(result_str, "3:1:%d:TypeId:SubTypeId:Block Size:", written);
		...
		if(tags)
		{
			...
			else
			{
				...
				written = write(h,localBuffer,sizeof(pt));
				memset(result_str, 0, 200);
        	                sprintf(result_str, "3:2:%d:%u:TypeId:SubTypeId:Tag Size:", written, tags->transaction_id);
				result_log (result_str);
				if(written != sizeof(pt)) return YAFFS_FAIL;
			}
		}
...
{
	//printf ("In yflash2_ReadChunkWithTagsFromNAND\n"); 
	int nread;
	int pos;
	int h;
	int localData = 0;
