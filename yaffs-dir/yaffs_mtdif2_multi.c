			      ...
			      const struct yaffs_ext_tags *tags)
{
	...
	yaffs_trace(YAFFS_TRACE_MTD, "nandmtd2_write_chunk_tags chunk %d data %p tags %p", nand_chunk, data, tags);
	...
	if (!dev->param.inband_tags) {
		...
		retval = mtd->write(mtd, addr, dev->param.total_bytes_per_chunk, &dummy, data);
/*
