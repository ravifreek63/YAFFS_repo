...
// dev - device, 
// nand_chunk - on disk address of chunk, 
// buffer  - destination to store the data
// tags - destination to read the tags in
			     ...
			     u8 *buffer, struct yaffs_ext_tags *tags)
{
	...
	if (dev->param.read_chunk_tags_fn)
		{
			result =
		    	dev->param.read_chunk_tags_fn(dev, flash_chunk, buffer,
						  tags);
			//if (tags != NULL)
			//printf ("In yaffs_rd_chunk_tags_nand writing to flash chunk = %d\n", flash_chunk);
		}
	else
	...
	if (tags && tags->ecc_result > YAFFS_ECC_RESULT_NO_ERROR) {
 	...
 	//printf ("In yaffs_rd_chunk_tags_nand the transaction id returned = %u\n", tags->transaction_id);
	return result;
}
...
int yaffs_wr_chunk_tags_nand(struct yaffs_dev *dev, int nand_chunk, const u8 *buffer, struct yaffs_ext_tags *tags)
{
	...
	//printf ("In yaffs_wr_chunk_tags_nand\n");
...
/*
 * yaffs_summary_add updates the summary tags for the device for the current nand chunk  
 */
